...
async def send_message_to_api(username, text):
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.post(f"{FASTAPI_URL}:{FASTAPI_PORT}/api/v1/message/", json=payload) as response:
...
async def get_messages_from_api(page: int):
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(f"{FASTAPI_URL}:{FASTAPI_PORT}/api/v1/messages/") as response:
